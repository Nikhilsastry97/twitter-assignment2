...
app.post("/register/", async (request, response) => {
  ...
  const userdatabaseDetails = await database.get(getUserQuery);

  if (userdatabaseDetails !== undefined) {
    response.status(400);
...
app.post("/login/", async (request, response) => {
  ...
  if (userdatabaseDetails !== undefined) {
    const isPasswordCorrect = await bcrypt.compare(
      password,
      userdatabaseDetails.password
    );

    if (isPasswordCorrect) {
      const payload = { username, userId: userdatabaseDetails.user_id };
